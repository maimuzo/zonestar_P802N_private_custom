...
void Commands::processMCode(GCode *com) {
    switch( com->M ) {
...
#if FIRST_TEST_ENABLE
		{
			...
			end_bit |= 0x20;
#endif			
			for(uint16_t codenum=1000; codenum>0; codenum--)
			{
#if (X_MIN_PIN > -1) && MIN_HARDWARE_ENDSTOP_X
				if(Endstops::xMin())
				{					
					UI_STATUS_UPD_RAM(UI_TEXT_TEST_XMIN_HIT);
					end_bit &= 0xfe;
				}
#endif				
#if (X_MAX_PIN > -1) && MAX_HARDWARE_ENDSTOP_X
				if(Endstops::xMax())
				{
					UI_STATUS_UPD_RAM(UI_TEXT_TEST_XMAX_HIT);
					end_bit &= 0xfd;
				}
#endif
#if (Y_MIN_PIN > -1) && MIN_HARDWARE_ENDSTOP_Y
				if(Endstops::yMin())
				{
					UI_STATUS_UPD_RAM(UI_TEXT_TEST_YMIN_HIT);
					end_bit &= 0xfb;
				}
#endif
#if (Y_MAX_PIN > -1) && MAX_HARDWARE_ENDSTOP_Y
				if(Endstops::yMax())
				{
					UI_STATUS_UPD_RAM(UI_TEXT_TEST_YMAX_HIT);
					end_bit &= 0xf7;
				}
#endif
#if (Z_MIN_PIN > -1) && MIN_HARDWARE_ENDSTOP_Z
				if(Endstops::zMin()){
					UI_STATUS_UPD_RAM(UI_TEXT_TEST_ZMIN_HIT);
					end_bit &= 0xef;
				}
#endif
#if (Z_MAX_PIN > -1) && MAX_HARDWARE_ENDSTOP_Z
				if(Endstops::zMax()){
					UI_STATUS_UPD_RAM(UI_TEXT_TEST_ZMAX_HIT);
					end_bit &= 0xdf;
				}
#endif
			...
			{
				HAL::delayMilliseconds(2000);
				UI_STATUS_UPD_RAM(UI_TEXT_TEST_ENDSTOP_ERROR);
			}		
		}
#endif	
		break;
